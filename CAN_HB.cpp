...
twai_frame_t tx_msg = {
    ...
    .buffer_len = sizeof(send_data), // This just specifies the lengt h of the data we're sending, which is 1 byte in this case
};
...
void setup() {
  ...
  // heartbeat_timer = timerBegin(100000000); // Create a hardware timer with a prescaler of 80 (1 tick = 1 microsecond)
...
void loop() {
  ...
  if(x > 80) x = 0;
  char text[16];
  sprintf(text, "Sent: %d", send_data[0]);
  controlPanel.setDisplayText("CAN_HB", text);
  memcpy(tx_msg.buffer, (uint8_t*)send_data, sizeof(send_data)); // Update the data field of the CAN message with the current value of x
  ESP_ERROR_CHECK(twai_node_transmit(twai_handle, &tx_msg,0));
  delay(100); 
} 